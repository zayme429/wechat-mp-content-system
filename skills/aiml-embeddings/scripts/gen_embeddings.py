...
DEFAULT_BASE_URL = os.getenv("EMBEDDING_BASE_URL", "https://api.aimlapi.com/v1")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--base-url", default=DEFAULT_BASE_URL, help="API base URL (default: from EMBEDDING_BASE_URL env or https://api.aimlapi.com/v1)")
    return parser.parse_args()

def load_api_key(args: argparse.Namespace) -> str:
    # 支持多种环境变量名（兼容国产模型服务商）
    api_key = os.getenv("EMBEDDING_API_KEY") or os.getenv("AIMLAPI_API_KEY") or os.getenv("OPENAI_API_KEY")
    if api_key: return api_key
    if args.apikey_file:
        key = pathlib.Path(args.apikey_file).read_text(encoding="utf-8").strip()
        if key: return key
    raise SystemExit("Missing API key. Set EMBEDDING_API_KEY or AIMLAPI_API_KEY or OPENAI_API_KEY")
...
def main() -> None:
    ...
    url = f"{args.base_url.rstrip('/')}/embeddings"
