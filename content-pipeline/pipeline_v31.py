...
"""
内容生产管线 v3.1 - 集成文章库
流程：主题确定 → 文献采集 → 选题设计 × 3 → 文章生成 × 3 → 保存到文章库 → 发送审核邮件
"""
...
sys.path.insert(0, str(Path(__file__).parent / 'article_library'))
...
class ContentPipelineV31:
    """内容生产管线 v3.1 - 集成文章库管理"""

    def __init__(self):
        self.config = self._load_json('config/pipeline.json')
        self.content_system = self._load_json('config/content_system.json')
...
直接输出文章正文，不要输出其他内容。"""
    ...
    def _score_quality(self, content: str) -> float:
        ...
        if '【' in content or '[' in content: score += 0.5
        return min(score, 10)

    # ─────────────────────────────────────────
    # Step 5: 保存到文章库 + 发送审核邮件
    # ─────────────────────────────────────────
    def save_to_library_and_notify(self, candidates: list, topic_info: dict):
        """
        保存候选到文章库并发送审核通知
        不再推送到草稿箱，只作标记存储
        """
        logger.info("=== Step 5: 保存到文章库 + 发送审核邮件 ===")
        ...
        # 获取用户邮箱
        user_email = self.secrets.get('review', {}).get('recipient', '')
        
        # 保存到文章库并发送通知
        try:
            article_ids = self.library.save_candidates_batch(
                topic=topic_info['topic'],
                candidates=candidates_data
            )
            logger.info(f"✅ 已保存 {len(article_ids)} 篇候选到文章库")
            ...
            if user_email:
                ...
                logger.info(f"✅ 审核通知已发送至 {user_email}")
            
            # 保存到本地数据库（备份）
            best = max(candidates, key=lambda x: x['quality_score'])
            self.db.save_article(
                date=today, topic=best['topic'],
                content=best['content'], candidates=candidates, status='in_library'
            )
            
            # 输出摘要
            library_link = self.library.get_library_link()
            print(f"\n{'='*50}")
            print(f"📚 文章库访问链接: {library_link}")
            print(f"\n候选文章分享链接:")
            for i, aid in enumerate(article_ids, 1):
                share_link = self.library.get_share_link(aid)
                print(f"  候选{i}: {share_link}")
            print(f"{'='*50}\n")
            
        except Exception as e:
            logger.error(f"❌ 保存到文章库失败: {e}")
            raise
    ...
    def run(self, manual_topic: str = None):
        logger.info("🚀 启动内容生产管线 v3.1（文章库版）")
        try:
            ...
            self.save_to_library_and_notify(candidates, topic_info)
            logger.info("✅ 管线完成")
            return True
        except Exception as e:
